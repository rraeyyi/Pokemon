...
## Function for berries
  * This function does not require any input arguments. The function loops through
the pokemon api and pulls the information for the 64 berries. Even its not directly link to pokemon data itself, it's still meaningful for future analytical purposes.  
...
# Subset dataframes for anaysis
  * We generated first 200 pokemon and all 64 berries from pokemon api through our functions. We noticed that some pokemon has dual-type. For convinience, we only include main type as our interest. Then we saved them as data frames to efficiently handle the actual exploratory data analysis.  
...
```
  ...
  * The other type pokemon's habitat also make lot of sense, where rock-type pokemon lives near mountain and sea, but they also can find near cave and forest. In that case, those pokemon might have duel-type based on their habitat.  
